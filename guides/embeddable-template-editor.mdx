...
<Note>
**Rate limit**: JWT endpoints support **120 requests per minute** for high-volume applications.
</Note>
...
Use [`comprehensive-template-update`](../api-reference/templates/comprehensive-template-update) when replacing the entire template resource.

**When to use**:
- Making major changes to the template
- Replacing all template properties
- Ensuring complete data consistency

**Requirements**:
- ✅ Must send all required fields
- ✅ Replaces the complete template
- ✅ Any fields not sent will be removed/reset
...
Use [`partially-update-template`](../api-reference/templates/partially-update-template) when updating specific fields only.

**When to use**:
- Updating only the template name
...
| Scenario | Use | Reason |
|----------|-----|--------|
| Saving all editor changes | PUT | Complete replacement ensures consistency |
| Updating template name only | PATCH | More efficient, safer |
| Changing specific settings | PATCH | Only affects targeted fields |
| Migrating template structure | PUT | Complete control over final state |
| User edits single field | PATCH | Preserves other properties |
| Bulk property updates | PUT | Clean slate approach |
