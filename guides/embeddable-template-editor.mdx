...
async function generateTemplateToken(templateId) {
  const response = await fetch('https://api.firma.dev/functions/v1/signing-request-api/jwt/generate', {
    method: 'POST',
    headers: {
...
def generate_template_token(template_id):
    response = requests.post(
        'https://api.firma.dev/functions/v1/signing-request-api/jwt/generate',
        headers={
...
<script src="https://app.firma.dev/embed/template-editor.js"></script>
...
<script>
  async function initializeEditor(templateId) {
    try {
      ...
      const jwt = await fetch('/your-backend-endpoint/generate-token?templateId=' + templateId)
        .then(res => res.json())
        .then(data => data.jwt);
      
      // Initialize the template editor
      const editor = new FirmaTemplateEditor({
        container: document.getElementById('template-editor-container'),
        jwt: jwt,
...
function TemplateEditorComponent({ templateId }) {
  ...
  useEffect(() => {
    ...
    script.src = 'https://app.firma.dev/embed/template-editor.js';
    script.async = true;
  ...
  useEffect(() => {
    async function fetchToken() {
      try {
        const response = await fetch(`/your-backend-endpoint/generate-token?templateId=${templateId}`);
        const data = await response.json();
        setJwt(data.jwt);
      } catch (error) {
...
| `onLoad` | function | Callback when editor loads successfully |
