...
Successful response (201) returns the `Team` resource with `id`, `name`, and timestamps. Example shape:
...
Response: the `Team` resource (see example above).
...
Response shape contains `results` (array of `Team`) and `pagination` metadata.
