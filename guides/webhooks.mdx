...
```bash
curl -X POST "https://api.firma.dev/v1/webhooks" \
...
Monitor your webhook's health using the API:

```bash
# List recent webhook events
curl "https://api.firma.dev/v1/webhook-events?webhook_id=wh_123&status=failed" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

Check the webhook's `consecutive_failures` field:

```bash
# Get webhook details
curl "https://api.firma.dev/v1/webhooks/wh_123" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
Retry individual failed events via API:

```bash
curl -X POST "https://api.firma.dev/v1/webhook-events/evt_123/retry" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
- [ ] Monitor `consecutive_failures` metric
...
- [List webhook events](../api-reference/webhook-events/list-webhook-events) for debugging
- [Test your webhook](../api-reference/webhooks/test-webhook) before going live