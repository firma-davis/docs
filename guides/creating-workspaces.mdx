...
```bash
curl -X POST "https://api.partner.com/v1/workspacess" \
...
```bash
curl "https://api.partner.com/v1/workspaces/workspace_123" \
  -H "Authorization: Bearer $FIRMA_API_KEY"
```
...
```bash
curl -X PUT "https://api.partner.com/v1/workspaces/workspace_123" \
  -H "Authorization: Bearer $FIRMA_API_KEY" \
  -H "Content-Type: application/json" \
