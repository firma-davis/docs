...
```bash
curl -X POST "https://api.firma.dev/functions/v1/signing-request-api/templates/tmpl_123/create-signing-request" \
...
async function createFromTemplate(templateId) {
  const resp = await fetch(`https://api.firma.dev/functions/v1/signing-request-api/templates/${templateId}/create-signing-request`, {
    method: 'POST',
    headers: {
...
def create_from_template(template_id):
    url = f"https://api.firma.dev/functions/v1/signing-request-api/templates/{template_id}/create-signing-request"
...
Use [`update-signing-request`](../api-reference/signing-requests/update-signing-request) or [`comprehensive-update-signing-request`](../api-reference/signing-requests/comprehensive-update-signing-request) when replacing the entire signing request.

**When to use**:
- Making major changes to the signing request
- Replacing all properties (recipients, fields, settings)
- Ensuring complete data consistency

**Requirements**:
- ✅ Must send all required fields
- ✅ Replaces the complete signing request
...
Use [`partially-update-signing-request`](../api-reference/signing-requests/partially-update-signing-request) when updating specific fields only.

**When to use**:
- Adding or updating specific recipients
- Updating metadata or custom fields
- Making targeted changes without affecting other properties
...
**Example** (Node.js):
```javascript
// Update only specific recipients
const response = await fetch(`https://api.firma.dev/functions/v1/signing-request-api/signing-requests/${signingRequestId}`, {
  method: 'PATCH',
  headers: {
    'Authorization': `Bearer ${process.env.FIRMA_API_KEY}`,
...
| Scenario | Use | Reason |
|----------|-----|--------|
| Complete signing request rebuild | PUT | Full replacement ensures consistency |
| Add/update one recipient | PATCH | More efficient, preserves other recipients |
| Update metadata only | PATCH | Only affects targeted fields |
| Change template reference | PUT | Major structural change |
...
<Note>
**Rate limit**: Sending signing requests supports **100 requests per minute**.
</Note>
...
- [Add automated reminders](../api-reference/signing-requests/get-signing-request-reminders) for pending recipients
- [Embed the template editor](embeddable-template-editor) with JWT authentication (120 req/min)
- [Configure workspace settings](workspace-settings) for custom email templates (100-200 req/min)
